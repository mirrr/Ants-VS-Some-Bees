...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        # BEGIN Problem 7
        #return not (self.place.ant or self.splace.ant and self.place.ant.blocks_path)
        # END Problem 7
...
class ThrowerAnt(Ant):
    ...
    food_cost = 3
    armor = 1
    min_range, max_range = 0, 9999999999
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor-amount<=0:
            amount += self.damage
        i = 0
        the_bees=list(self.place.bees)
        while i<len(the_bees):
            self.place.bees[i].reduce_armor(amount)
            i+=1
        Insect.self.reduce_armor(amount)
        # END Problem 5
...
def apply_effect(effect, bee, duration):
    ...
    # BEGIN Problem EC
    "*** YOUR CODE HERE ***"
    og_colonytime = colony.time
    while (colony.time - og_colonytime)<duration:
        effect(bee.action)
